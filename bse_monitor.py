...
class BSEMonitor:
    ...
    def extract_category(self, cells) -> str:
        """Extract announcement category."""
        # This would need to be customized based on BSE's actual structure
        return "General"

    def extract_xbrl_url(self, row) -> Optional[str]:
        """Extract XBRL file URL from the row."""
        links = row.find_all('a', href=True)
        for link in links:
            href = link['href']
            if 'xbrl' in href.lower() or 'xml' in href.lower():
